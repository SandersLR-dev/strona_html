...
try
{
    ...
    {   
        ...
        if($rezultat=$polaczenie->query("SELECT * FROM korepetycje WHERE nauczycielid ='$id' OR uczenid ='$id'"))
        {
  ...
  <div class="w3-row">
    ...
    <div class="middle">
        <div class="w3-container">
         <h4 class="w3-center">Twoje spotkania</h4>
         <p class="w3-center"><img src="calendar.png" class="w3-circle" style="height:106px;width:106px" alt="Avatar"></p>
                ...
                foreach($wyniki as $wynik){
                    ...
                    $nauczycielid=$wynik['nauczycielid'];
                    $uczenid = $wynik['uczenid'];
                    $pid = $wynik['przedmiotid'];
                
                //$nID = htmlspecialchars($wynik['UserID']);
                    
                    $rezultat=$polaczenie->query("SELECT * FROM user WHERE UserID='$nauczycielid'");
                    $nauczyciel=$rezultat->fetch_assoc();
                    
                    $rezultat=$polaczenie->query("SELECT * FROM user WHERE UserID='$uczenid'");
                    $uczen=$rezultat->fetch_assoc();
                    
                    $rezultat=$polaczenie->query("SELECT * FROM przedmiot WHERE PrzedmiotID='$pid'");
                    $pname=$rezultat->fetch_assoc();
                
                
                   
                    echo '<div class="blok" >';
                    echo '<p class="w3-center"><img src="men.png"  class="w3-circle  w3-border-black" style="height:60px;width:60px;border:1px solid black" alt="Avatar"></p>';
                    echo '<h6  class="w3-center" class="w3-border w3-padding">';
                    echo "Przedmiot : </br>".$pname['Nazwa'];
                    echo '</h6>';
                    echo '<p class="w3-center" class="w3-border w3-padding">';
                    
                    echo "Nauczyciel : </br>".$nauczyciel['Imie']." ".$nauczyciel['Nazwisko']."</br>";
                    echo "Uczeń : </br>".$uczen['Imie']." ".$uczen['Nazwisko']."</br>";
                    echo "Data : ".$wynik['data']."</br>";
                    echo "Godzina startu : </br>".$wynik['godzstart']."</br>";
                    echo "Godzina końca : </br>".$wynik['godzkoniec']."</br>";
                    echo $wynik['opis']."</br>";
                    echo '</p>';
